...
@include('crud::fields.inc.wrapper_start')
    ...
    <p>
        <br/>
        <img id="preview_{{$field['name']}}" src="{{ url(old(square_brackets_to_dots($field['name'])) ?? $field['value'] ?? $field['default'] ?? '') }}" width="200px" height="auto"  alt=""/>
    </p>
	@if (isset($field['hint']))
    ...
    @push('crud_fields_scripts')
		...
		<script type="text/javascript">
			...
			function processSelectedFile(filePath, requestingField) {
                let fieldUrl = filePath.replace(/\\/g,"/")
				elfinderTarget.val(fieldUrl);
                //elfinderTarget.parentNode.siblings('.p_preview').children('.img_preview').src = '/' +fieldUrl;
                let eleName = elfinderTarget.attr('name');
                document.getElementById('preview_' + eleName).src = '/' +fieldUrl;
